 ...
 * Loaded once on first call, then reused.
 */
let _companyDirs = null;

function getCompanyDirs() {
  if (_companyDirs) return _companyDirs;
  try {
 ...
 *
 * Returns:
 *   { content, company, role, matchType } on success
...
export function lookupResearch(companyName, jobTitle) {
  ...
  const content = loadResearchFile(companyMatch.directory, roleFile);
