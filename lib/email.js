...
export async function sendPlaybookEmail(email, name, pdfBuffer, type) {
  ...
  const filename = isOffer
    ? `NegotiateUp-Offer-Playbook-${name.replace(/\s+/g, '-')}.pdf`
    : `NegotiateUp-Raise-Playbook-${name.replace(/\s+/g, '-')}.pdf`;
...
<body style="margin:0;padding:0;background:#faf8f5;font-family:Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:40px 20px;">
    ...
    <div style="text-align:center;margin-bottom:32px;">
      ...
      <span style="font-size:20px;font-weight:bold;color:#1a1a1a;"> NegotiateUp</span>
    </div>
    ...
    <div style="text-align:center;margin-top:24px;">
      <p style="color:#9ca3af;font-size:12px;margin:0;">
        © ${new Date().getFullYear()} NegotiateUp. You received this because you purchased a playbook.
      </p>
    </div>
  </div>
</body>
  ...
  const { data, error } = await resend.emails.send({
    from: process.env.FROM_EMAIL || 'NegotiateUp <playbook@negotiateup.com>',
    to: email,
    subject,
    html,
    attachments: [
